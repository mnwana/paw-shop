...
export default function Reply({commentId, replyId, replierUsername, content, dateCreated}){
    return <>
        <div className="reply rounded border border-top-0 px-2 pt-3 pb-0">
            ...
            <div className="reply-meta-and-delete-btn-wrapper d-flex align-items-center mx-1 mx-md-2     mb-1">
                <DeleteBtn commentId={commentId} replyId={replyId} />

                <p className="reply-meta mb-0 ps-2">
                <Link to={`/user/${replierUsername}`} className="username-meta hover-opacity">{replierUsername}</Link><span className='border-end border-dark'>&ensp;</span>&ensp;
